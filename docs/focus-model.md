...
| 12 | `SyncStatusIndicator` (sidebar footer) | `core/widgets/sync_status_indicator.dart` | no | no | Story 4.4 shipped the green/amber/grey dot palette + the in-progress `_PulsingDot` (vanilla `AnimationController` + `FadeTransition`, reduce-motion gated via `MediaQuery.disableAnimations`). Still NOT a focus surface — Tab passes through. The collapsed-mode `Tooltip` is hover-only (Flutter built-in); no keyboard surface added. The `Semantics(liveRegion: true)` wrap announces status transitions; the dot itself is `ExcludeSemantics`. Story 4.5 confirmed the indicator stays read-only — disconnect lives in Settings, not the sidebar. |
| 13 | Settings Disconnect confirmation | `features/settings/presentation/settings_screen.dart` `_DisconnectConfirmation` | yes (Flutter-managed `FilledButton` autofocus) | yes (button self-claim on tap) | Story 4.5. Inline confirmation row that replaces the default `[Disconnect]` OutlinedButton on tap. Tab order: `[Disconnect]` FilledButton (autofocused on open, so Enter commits the destructive action) → `[Cancel]` TextButton. Local `Shortcuts({Esc: DismissIntent()}) + Actions({DismissIntent: → setState(showConfirmation = false)})` wrap so Esc dismisses the confirmation without bubbling to the AppShell's `AppDismissIntent` cascade — the local handler consumes the event so Esc outside the confirmation continues to trigger the cascade (e.g. clearing selected bookmark). Pattern reference: `folder_tree.dart` Story 2.4 cascade-delete inline confirmation. |
