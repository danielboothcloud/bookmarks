...
| 12 | `SyncStatusIndicator` (sidebar footer) | `core/widgets/sync_status_indicator.dart` | no | no | Informational sidebar-footer widget added in Story 4.2. NOT a focus surface — Tab passes through. A `Semantics(liveRegion: true)` wrap announces status transitions to screen readers. Story 4.4 may upgrade to an interactive variant (click-to-retry / tooltipped); when that lands, update this row's two `no` columns. |
